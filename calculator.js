...
function buttonClick(event) {
     //values for clicked buttons
    const target = event.target;
    const action = target.dataset.action;
    const value = target.dataset.value;

    //console.log(target, action, value);
   //Switch case to control caculator
   ...
   switch (action) {
    case 'number':
        ...
        case 'backspace':
            backspace();
        break;
    //Add the result to an experssion as a starting point if expression is empty
    case 'addition':
        case 'substraction':
        case 'multiplication':
        case 'division':
...
//check if this is the first decimal in the current number or if the experssion is empty
...
function negate() {
    ...
    if (expression === '' && result !== '') {
    result = -result;
    //Toggle the sign of the expression if its not alredy nageative its not empty

    } else if (!expression.startsWith('-') && expression !== '') {
        expression = '-' + expression;
        //Remove the negation sign for the expression if its alredy negative
...
function percentage() {
    //evaluate the expression hence it will take only the percenteage of the first number
    if (expression !== '') {
        result = evaluateExpression();
        expression = '';
    if (!isNaN(result) && isFinite(result)) {
        ...
        //if expression is empty and result exist divide by 100
